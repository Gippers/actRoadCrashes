...
gt(msep_results, rowname_col = "row"
  ...
  fmt_markdown(columns = stub()) |>
    cols_label(
    pred.seacf = "Time-Trend ARMA(0, 1)", 
    pred.aic = "Time-Trend ARMA(1, 0)", 
    pred.sar = md("$\\text{SARIMA}(2,0,1)(1,0,1)_7$"), 
    pred.asar = md("$\\text{SARIMA}(1,0,0)(2,0,0)_7$")
  )
# TODO up to here
# TODO check the formatting, might need to be all expressions to make it look ok.
# TODO should this be vertical? 
```
...
gt(crash.train[DATE %in% tt01.outlier])
```
...
gt(crash.train[DATE %in% tt10.outlier])
```
...
gt(crash.train[DATE %in% tsar.outlier])
```
...
gt(crash.train[DATE %in% asar.outlier])
```