...
This report will examine two different types of time series modelling methods for forecasting ACT daily road crashes. The first are time trend models, which effectively fit a regression model to the data. The advantage of this approach is that it is relatively simple and we can easily account for the seasonal components by including them as factor variables. However, using a regression model with time series data will almost always lead to the assumption of independence being violated as each point in the time series will have some dependence on previous time periods. To overcome this, ARIMA models will be used to model the residuals. In addition, Seasonal ARIMA (or SARIMA) models will also be fit to the ACT road crash data. SARIMA models are particularly well-suited for time series with seasonality because they can explicitly capture both the regular patterns that repeat at fixed intervals (like weekly or monthly cycles in crash data) and the underlying trends in the data simultaneously. In addition, packages exist for automatically fitting SARIMA models which simplifies the process of applying this model type.
