...
```{r debug-paths, echo=FALSE}
cat("=== DEBUG: PATHS & FILES ===\n")

cat("getwd():\n")
print(getwd())

cat("\nRecursive file listing from working directory:\n")
print(list.files(".", recursive = TRUE, full.names = TRUE))

cat("\nDoes the crash CSV exist (relative path)?\n")
print(file.exists("data/ACT_Road_Crash_data_20250430.csv"))

cat("\nnormalizePath for crash CSV:\n")
print(normalizePath("data/ACT_Road_Crash_data_20250430.csv", mustWork = FALSE))
...
```{r}
#| label: fig-pred
#| fig-cap: "Model Test Prediction vs Actual"

# TODO use an interactive graph
...
plot(ts(crash.test$NUM_CRASHES,start = 912 ), type = "l", lwd = 2, ylim = range(c(crash.test$NUM_CRASHES, crash.test$pred.seacf)), 
     main = "", ylab = "ACT Daily Road Crashes", bty = "l")
