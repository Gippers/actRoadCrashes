...
Observing the histogram of ACT daily road crashes (@fig-crash-hist), we observe that the distribution has a longer right tail suggesting the data is slightly right skewed. Given that road crashes are also count data and we observe a right skewed distribution, we can conclude the data has a Poisson distribution. As a result it is worth trialing a poisson error assumptions for the model however, it is not dissimilar to a normal distribution so linear regression may still be appropriate assumption.
...
We observe several patterns from the time series graph @fig-tsplot.  The trend of the time series changes over time, with a slight decrease in the second half of the time series. This introduces a challenge as for the train/test split, the model will be trained on the first half of the data set only which appears to in fact have a stationary mean. This is unavoidable though as the test data set is treated as unobserved for the purposes of the modelling. Therefore, from here on out the EDA will focus on the  $1 \ldots T$ time points as this will guide later model selection. Certain periods in the dataset have decreased variance such as January, which signals the presence of non-constant variance. There is evidence of seasonality, for example around January there is a drop in the number of road crash reports. Finally, when zooming in on the graph (which can be accomplished through the zoom in or by dragging and selecting a portion of the data) there appears to be some weekly seasonality.
